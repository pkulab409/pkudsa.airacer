...
| `tests/test_validator.py` | validator 的 pytest 套件，**20 用例全绿**；覆盖规则正反样本、结构化输出、性能 meta |
| `docs/manual/local_test_guide.md` | 面向学生的中文手册 v0.1：定位/环境/5 分钟上手/提交前自查/FAQ/错误码速查 |
...
- `sdk/README_LOCAL.md`：同步 `run_local.py`、`rules.yaml`、`examples/` 到文件清单，补 Option A/B 两种工作流
...
├── rules.yaml                      # 🆕 校验规则
├── check_env.py
├── make_local_config.py
├── validate_controller.py          # 🔄 重写升级
├── run_local.py                    # 🆕 一键启动
├── team_controller.py              # 官方最小模板
├── example_controller.py           # numpy 循线示例
└── examples/
    └── team_controller_tutorial.py # 🆕 PID 教学版

docs/manual/local_test_guide.md     # 🆕 中文手册

tests/test_validator.py             # 🆕 20 个 pytest 用例
```
...
| 占位项 | 依赖角色 | 说明 |
|---|---|---|
| `docs/manual/local_test_guide.md` FAQ Q4 / 反馈渠道 | 课程组 | 官方 issue/群渠道定了再填 |
| `airacer.wbt` 是否做"学生简化版" | 测试同学 | 当前直接复用 `simnode/webots/worlds/airacer.wbt`；如要专用简化赛道再补 |
| Backend `submission.py::_validate_code` 的 TODO "与SDK代码审查部分保持一致" | 后端同学 | **本 PR 未触碰后端**（用户指定 `rules.yaml` 只管 SDK 作用域）；后续若后端要对齐，可改为 `from sdk.validate_controller import validate` 复用本模块 |
| `simnode/car_sandbox.py` 与 `sandbox_runner.py` 黑名单轻微不一致（Windows 项 `winreg`/`nt` 等） | Sim Node 同学 | 本 PR 的 `rules.yaml` 已做并集，但线上两处仍各自维护，建议后续以 `rules.yaml` 为准做一次对齐 |
...
1. **规则设计合理性**：`sdk/rules.yaml` 的黑白名单是否与线上沙箱口径一致？（对比 `simnode/car_sandbox.py::_BLOCKED_PREFIXES` 和 `sandbox_runner.py::BLOCKED_PREFIXES`）
2. **退出码契约**：`0/1/2/3` 的划分是否清晰、能被 CI 正确消费？
3. **动态加载的副作用**：`_load_module` 用 `sys.meta_path.insert(0, hook)` + `try/finally remove`，reviewer 请检查异常路径是否会泄漏 hook
...
**提交人**：AI SDK 组
**请求合入**：目标分支 `main`，需 1 位 reviewer 通过
