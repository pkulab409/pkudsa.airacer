...
function drawCars(ctx, frame, W, H) {
  ...
  frame.cars.forEach((car, i) => {
    ...
    const color =
      car.status === "disqualified" || car.status === "idle"
        ? "#555"
    ...
    // Direction arrow
    // heading from supervisor: atan2(-sin(θ), cos(θ)), so Y (north/south) is inverted.
    // Use +car.heading to correct the sign, then offset by π/2 because the
    // arrow triangle is drawn pointing UP (negative canvas Y).
    ctx.save();
    ctx.translate(cx, cy);
