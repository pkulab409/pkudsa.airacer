...
| 世界文件 | `sdk/webots/worlds/{track_basic,track_complex,airacer}.wbt`（`--world` 切换） | 由赛事配置指定 |
...
> 💡 **SDK 是自包含的**：Webots 资产（赛道/车型 PROTO/控制器）已经全部内置到 `sdk/webots/` 下。如果你只想拿到 SDK，只拷贝 `sdk/` 这一个目录即可运行本地测试（仅需 `sdk/` + Python 环境 + Webots）。
...
├── car_sandbox.py                  # 线上沙箱黑白名单源头（一致性测试用）
├── check_env.py                    # 环境自检脚本
...
├── examples/
│   └── team_controller_tutorial.py # 🎓 教学版（OpenCV 示例，PID 循线，有详细注释）
└── webots/                         # 🆕 Webots 资产（内置，无需从其他地方拷贝）
    ├── worlds/                     # 赛道 .wbt：track_basic / track_complex / airacer
    ├── protos/                     # 6 个 Car 系列 PROTO + CAR_UPGRADE_SUMMARY.md
    └── controllers/                # 官方 Webots 控制器
        ├── car/                    #   车端：拉起 sandbox_runner 并交换帧
        └── supervisor/             #   裁判端：检测检查点/碰撞/遥测录制
```
...
2. 生成 `sdk/.local/race_config.json`（`sdk/` 内部的隐藏子目录，全 ASCII，跟 Webots 友好）
3. 找到 Webots 可执行文件并启动默认赛道 `sdk/webots/worlds/track_basic.wbt`，
...
目前 `sdk/webots/worlds/` 里有三个预置赛道，每个赛道提前摆好了若干辆不同外观的 Car PROTO（CarPhoenix、CarThunder …），你通过 `--world` 选赛道、`--car-slot` 选车位，即可指定自己被分配到哪辆车：
...
python3 sdk/run_local.py --code-path sdk/my_controller.py --world basic --car-slot car_1
...
的是**运行时沙箱**（`sdk/car_sandbox.py` / `sdk/webots/controllers/car/sandbox_runner.py`
里的 `SandboxImportHook`）——也就是说：提交的那一刻可能通过，但线上真正跑比赛时才 ImportError。
...
A：所有预置赛道都放在 `sdk/webots/worlds/` 下：
...
车辆 PROTO 在 `sdk/webots/protos/Car*.proto`（CarPhoenix / CarThunder / CarViper / CarNova / CarFrost / CarShadow 共 6 种外观）。
...
`sdk/make_local_config.py` 已经做了兜底：当 `sdk/` 所在路径含非 ASCII 字符时，`recording_path` 会自动落到 `%TEMP%\airacer_local\recordings`（Windows 的 `%TEMP%` 对非 ASCII 用户名会返回 8.3 短路径，全 ASCII）。所以**现在**直接运行 `run_local.py` 不会再报错。
