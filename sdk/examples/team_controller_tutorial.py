"""OpenCV tutorial controller.

This example uses `cv2` to preprocess images and demonstrates a small
but robust lane-following + obstacle avoidance controller suitable for
students who want to use OpenCV APIs.

Copy this file to `sdk/my_controller.py` and run `python sdk/run_local.py`.
"""
...
def _estimate_lane_center_cv(bgr: np.ndarray) -> float | None:
    ...
    if HAVE_CV2:
        ...
        # numpy fallback: simple grayscale + column sum centroid
